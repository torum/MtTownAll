...
public class PostalCodeDataService : IPostalCodeDataService
{
    ...
    private static List<PostalCode> PostalCodes(SqliteConnectionStringBuilder connectionStringBuilder, string postalCode)
    {
        ...
        try
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    MunicipalityCode = Convert.ToString(reader["municipality_code"]),
                    Code = Convert.ToString(reader["postal_code"]),
                    PrefectureName = Convert.ToString(reader["prefecture_name"]),
                    SikuchousonName = Convert.ToString(reader["sikuchouson_name"]),
                    ChouikiName = Convert.ToString(reader["chouiki_name"])
                };
